...
oc create secret generic products-ms-secret --type=Opaque \
...
kubeseal -n ${SECRET_NAMESPACE} --controller-name=${SEALED_SECRET_CONTOLLER_NAME} --controller-namespace=${SEALED_SECRET_NAMESPACE} -o yaml < DELETE_ME.yaml > products-ms-sealed-secret.yaml
